...
void init_sample_users() {
    ...
    strcpy(admin->name, "admin");  strcpy(admin->password, "admin123");
    ...
    strcpy(svc->name, "service");  strcpy(svc->password, "123456");
    ...
    strcpy(wh->name, "warehouse");  strcpy(wh->password, "123456");
    ...
    strcpy(dp->name, "dispatcher");  strcpy(dp->password, "123456");
    ...
    strcpy(u1->name, "user1");  strcpy(u1->password, "123456");
...
int add_user(const char *name, const char *pwd, UserRole role) {
    ...
    u->name[NAME_LEN - 1] = '\0';
    strncpy(u->password, pwd, PASSWORD_LEN - 1);
...
int bin_save_list(const char *filename,
	...
	while(p){
		count ++;
		p=*(const char **)(next_offset);//通过便宜获取next指针 
	} 
